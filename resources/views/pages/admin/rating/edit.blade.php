...
@section('content')
    <div class="container-fluid px-4 border-bottom">
        <h1 class="mt-4 h2">Edit data {{ $data->nama_konsumen }}</h1>
    </div>

    <form class="col-lg-8 contianer-fluid px-4 mt-3" method="POST" action="{{route('admin.konsumen.update', $data->id_konsumen)}}"
        enctype="multipart/form-data">
        @method('PUT')
        @csrf
        <div class="mb-3">
            <label for="nama_konsumen" class="form-label">Nama Lengkap</label>
            <input type="text" class="form-control @error('nama_konsumen') is-invalid @enderror" id="nama_konsumen" name="nama_konsumen"
                value="{{ old('nama_admin', $data->nama_konsumen) }}" autofocus required placeholder="Masukan nama lengkap">
